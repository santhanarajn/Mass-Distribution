...
def B_radius(B1, Theta1, B2, Theta2, a1, a2, a, T):
    if temp_dependent:
        R1, R2, CA, Rt, R, b = T_radius(a1, a2, a, T)
    else:
        R1, R2, CA, Rt, R, b = Radius(a1, a2, a)
    C1 = R1 * (1 + B1 * (0.5 * (3 * np.cos(Theta1) ** 2 - 1)))
    C2 = R2 * (1 + B2 * (0.5 * (3 * np.cos(Theta2) ** 2 - 1)))
    ct = C1 + C2
    R = ct
    return C1, C2, CA, ct, R, b
...
def B_coulomb(B1, B2, Z1, Z2, R, C1, C2):
    """Deformation dependent coulomb energy"""
    V0 = (1.44 * Z1 * Z2) / R
    V1 = (3 / (2 * np.sqrt(5 * np.pi) * R ** 2)) * ((C1 ** 2) * B1 + (C2 ** 2) * B2)
    V2 = (3 / (7 * np.pi * R ** 2)) * ((C1 ** 2) * (B1 ** 2) + (C2 ** 2) * (B2 ** 2))
    V3 = (9 / (14 * np.pi * R ** 4)) * ((C1 ** 4) * (B1 ** 2) + (C2 ** 4) * (B2 ** 2))
    V4 = ((27 * (C1 ** 2) * (C2 ** 2)) / (10 * np.pi * R ** 4)) * B1 * B2
    Vc = V0 * (1 + V1 + V2 + V3 + V4)
    return Vc
...
def B_proximity(A1, A2, A, Z1, Z2, B1, B2, T):
    ...
    R_p1, R_p2, _, _, _, _ = B_radius(B1, (np.pi / 2), B2, (np.pi / 2), a1, a2, a, T)
    R_z1, R_z2, _, _, _, _ = B_radius(B1, 0, B2, 0, a1, a2, a, T)
...
for i in range(l, k + 1, 1):
        ...
        if not condition1.empty and not condition2.empty:
            ...
            if deformation:
                ...
                if choice3 == 5:
                    ...
                    if not condition_B1.empty and not condition_B2.empty:
                        B1 = condition_B1['B2'].values[0]
                        B2 = condition_B2['B2'].values[0]
                    elif not condition_R1.empty and not condition_R2.empty:
                        B1 = condition_R1['B2'].values[0]
                        B2 = condition_R2['B2'].values[0]
                    else:
                        B1 = 0
                        B2 = 0
                C1, C2, CA, ct, R, b = B_radius(B1, Theta1, B2, Theta2, a1, a2, a, T)
            else:
                if temp_dependent:
            ...
            ETA = masstrans(C1, C2, R, a, a1, a2, fr)[0]
            BNN = masstrans(C1, C2, R, a, a1, a2, fr)[1]

            if deformation:
                Ec = B_coulomb(B1, B2, z1, z2, R, C1, C2)
                Vp = B_proximity(a1, a2, a, z1, z2, B1, B2, T)
            else:
                Ec = coulomb(z1, z2, R)
            ...
            if deformation:
                frag.append({'A1': a1, 'Z1': z1, 'N1': abs(a1 - z1), 'EL1': condition2['EL'].values[0],
                             ...
                             'Beta1': B1, 'Beta2': B2, 'BE1': BEF1, 'BE2': BEF2, 'Ec': Ec, 'Vp': Vp,
    ...
    pot_file = fr'v{a}{e.strip()}_t{T}l{l:02d}be.out'         # Fragmentation potential file
    out_file = f"P0_{a}{e.strip()}_t{T}l{l:02d}be.out"        # Output file
    with open(pot_file, 'w') as file:
        file.write(f"{a}  {a}\n{a - 1}  {a - 1}  10\n{diff}\n")
        for index, row in v.iterrows():
            file.write('\t'.join(map(str, row)) + '\n')
mass_file = fr'm{a}{e.strip()}_t{T}be.out'                    # Mass parameter file
beta.to_csv(mass_file, mode='w', index=False, header=False, sep='\t')

with pd.ExcelWriter(f'out_{a}_{e.strip()}l{l:02d}_t{T}be.xlsx') as writer:
